...
| 2026-06-15 | phase4-export-dispersion-curves-v1 | Implemented `scripts/phase4_picking/export_dispersion_curves.py`. Converts `predictions.npz` to physical units (Hz, 1/m, phase velocity) with first-order uncertainty propagation. Exports per-spectrum CSV/JSON plus a combined `all_dispersion_curves.csv` and `manifest.json`. Supports `--min-certainty`, `--skip-absent`, and `--certainty-strategy`. | N/A | 1,392 spectra / 178,915 picks exported from `phase4-picking-seq-bilstm-v1` | ✅ Inversion-export pipeline complete |
