...
class PseudoLabelTrainer:
    """Trainer for supervised classification on pseudo-labels.

    Supports both MLP (feature-input) and CNN (raw-spectrum-input) classifiers.
    Uses AMP, gradient accumulation, cosine warmup schedule, and checkpointing
    consistent with the MAE trainer pattern.
    """
    ...
    def __init__(
        ...
        Args:
            model: Classifier model (``MLPClassifier`` or ``ShallowCNNClassifier``).
            config: Training configuration dataclass with attributes:
                ``lr``, ``betas``, ``weight_decay``, ``epochs``,
                ``warmup_ratio``, ``accum_steps``, ``grad_clip_norm``,
                ``seed``, ``log_interval``, ``pin_memory``.
            device: Torch device.
    ...
    def _train_epoch(self, epoch: int) -> dict[str, Any]:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            # Support both (x, y) and (x, y, metadata) loaders
            x = batch[0].to(self.device, non_blocking=self.config.pin_memory)
            y = batch[1].to(self.device, non_blocking=self.config.pin_memory)
            ...
            # Un-scale loss for logging
            total_loss += loss.item() * self.config.accum_steps
            num_batches += 1

            # Accuracy (unscaled logits)
            with torch.no_grad():
            ...
            if batch_idx % self.config.log_interval == 0:
                ...
                logger.debug(
                    "Epoch %d | Batch %d/%d | loss=%.4f | acc=%.2f%% | lr=%.2e",
                    epoch + 1,
                    batch_idx,
                    len(self.train_loader),
                    loss.item() * self.config.accum_steps,
                    (correct / total_samples) * 100 if total_samples > 0 else 0.0,
                    current_lr,
                )
    ...
    def _validate(self, epoch: int) -> dict[str, Any]:
        ...
        # Silhouette score on penultimate embeddings
        if all_embeddings:
            ...
            try:
                ...
                sil = silhouette_score(embeddings_cat, labels_cat)
                metrics["val_silhouette"] = float(sil)
                logger.info("Validation Silhouette: %.3f", sil)
            except Exception:
